  ...
  const updateDemoAccountBalance = async (profitOrLoss) => {
    try {
      ...
      if (demoAccountData) {
        ...
        await updateDoc(doc(db, "users", currentUser.uid, "accounts", "Real"), {
          balance: newBalance,
        });

        console.log("Real account balance updated:", newBalance);
      } else {
        console.log("Real account data not found.");
      }
    } catch (error) {
      console.error("Real updating demo account balance:", error);
    }
  };
...
const handleTrade = (action) => {
  ...
  const updateFirebaseBalance = async () => {
    ...
    try {
      ...
      if (!demoAccountData) {
        console.error('Real account data not found.');
        alert('Real account data not found.');
        return;
      }
